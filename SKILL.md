...
记忆库为**全局库** D:\dsh\home\memories（默认 $DSH_HOME/memories，环境变量 MEMORY_HOME 可覆盖为项目级库），已收录 gongneng 与 260815 全部历史会话的结构化摘要。入口脚本 D:\dsh\260815\mem.py，全部命令通过 pwsh 执行：python D:\dsh\260815\mem.py <命令>。
...
| 写常驻事实 | python D:\dsh\260815\mem.py mem add "事实文本"；查看 mem show；删除 mem rm "唯一子串" |
...
单次注入 ≤ 3 条检索结果（约 2k token）；MEMORY.md 全程 ≤ 2200 字符。存储与检索本身 0 token，只有注入才花钱。